...
function deleteThisComment(comment_id) 
{
	...
	if(confirm("Are you really sure that you want to delete this comment? click on Ok if Yes and cancel if No."))
	{
		...
		$.ajax({
			type: "POST",
			url: "vasplus_programming_blog_PHP.php",
			data: dataString,
			cache: false,
			beforeSend: function() 
			{
			...
			success: function(response) 
			{
				...
				if(response != "")
				{
					$("#display_posted_comments_by_vasplus_programming_blog").hide().fadeIn('slow').html(response);
				}
				else
				{
...
function Post_Comment_By_Vasplus_Programming_Blog() 
{
	...
	$("#display_posted_comments_by_vasplus_programming_blog").show();
	
	if(posted_comment == "" || posted_comment === "What's on your mind?")
	{
		$("#display_posted_comments_by_vasplus_programming_blog").html('<div class="info">Please type your comment in the required field to post a comment.</div>');
		$("#posted_comment").focus();
	}
	else if(fullname_id == "" || fullname_id === "Fullname")
	{
		$("#display_posted_comments_by_vasplus_programming_blog").html('<div class="info">Please enter your fullname in the required field to proceed.</div>');
		$("#fullname_id").focus();
	}
	else if(email_id == "" || email_id === "Email Address")
	{
		$("#display_posted_comments_by_vasplus_programming_blog").html('<div class="info">Please enter your email address in the required field to proceed.</div>');
		$("#email_id").focus();
	}
	else if(reg.test(email_id) == false)
	{
		$("#display_posted_comments_by_vasplus_programming_blog").html('<div class="info">Please enter a valid email address to proceed.</div>');
		$("#email_id").focus();
	}
	else if(page_url == "")
	{
		$("#display_posted_comments_by_vasplus_programming_blog").html('<div class="info">Sorry, the identity of this page could not be verified at the moment. Please try refresh this page and try again or contact the admin of this website to report this error message if the problem persist. Thanks.</div>');
	}
	else 
	{
		...
		$.ajax({
			...
			beforeSend: function() 
			{
				$("#display_posted_comments_by_vasplus_programming_blog").html('<br clear="all"><div style="font-family:Verdana, Geneva, sans-serif; font-size:12px;color:gray;">Please wait <img style="" src="images/loadings.gif" align="absmiddle" alt="Loading" /></div>');
			},
			success: function(response) 
			{
				...
				{
					$("#display_posted_comments_by_vasplus_programming_blog").html(response);
				}
				else
				{
					$("#display_posted_comments_by_vasplus_programming_blog").html('');
					$("#display_posted_comments_by_vasplus_programming_blog").hide();
					...
					$("#email_id").hide();
					$("#comment_logout").show();
					$("#vasplusProgrammingBlogCommentsPosted").prepend($(response).fadeIn(400));
					$("#posted_comment").Watermark("What's on your mind?");
				}
			}
		});
...
$("#posted_comment").live("keypress",function() { $("#display_posted_comments_by_vasplus_programming_blog").html(''); });
$("#fullname_id").live("keypress",function() { $("#display_posted_comments_by_vasplus_programming_blog").html(''); });
$("#email_id").live("keypress",function() { $("#display_posted_comments_by_vasplus_programming_blog").html(''); });
